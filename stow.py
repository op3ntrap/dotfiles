...
def stow(package):
    ...
    for val in package:
        ...
        if (os.environ['PWD'] == dotfiles_repo):
            ...
            if (exit_code == 0):
                log[val] = {"status": "success", "output": cmd_out}
            else:
                log[val] = {"status": "failed", "output": cmd_out}
        else:
            print("Please change to the repo directory and run this command")

            return

    # Give info if there were some errors or tell me if it was a success
    errors = 0

    for val in log:
        if (log[val] == "failed"):
            print(
                "Some Packages failed to update. Please check the latest log "
                + "file for errors"
            )
            errors += 1
    ...
    # Write the errors to the log file or print the errors to the terminal if
    # verbose flag is given.
    logfile = open('./.StowLogs/.stowlog ' + str(datetime.datetime.now()),
                   'w+')
